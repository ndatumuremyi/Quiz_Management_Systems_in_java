...
label1.setPadding(new Insets(1,4,15,50));
 ...
 border.setCenter(getNav());
                            ...
                            HBox hbb=new HBox();
                            hbb.getChildren().addAll(new ImageView("quizes/image/user.png"),new Label(new GetUserName().getUser()));
                            border.setLeft(hbb);
...
logout.setOnMousePressed(e->{
  ...
  if(a==0)
  {
      ...
      primarStage.close();
      quizes.LoginForm back=new quizes.LoginForm();
      Stage st=new Stage();
      back.start(st);
  }
});
